...
public:
  ...
  AirportCounter(TIterator begin, TIterator end) {
	  for (auto iter = begin; iter != end; ++iter)
	    {
			++airports[static_cast<uint32_t>(*iter)];
	    }
  }

  size_t Get(TAirport airport) const {
	  return airports.at(static_cast<uint32_t>(airport));
  }
  ...
  Items GetItems() const {
	  std::array<Item, static_cast<uint32_t>(TAirport::Last_)> result;
	  for (uint32_t i = 0; i < static_cast<uint32_t>(TAirport::Last_); ++i)
	  {
		  result[i] = std::make_pair(static_cast<TAirport>(i), airports.at(i));
	  }
	  return result;
  }

private:
  std::array<size_t, static_cast<uint32_t>(TAirport::Last_)> airports = {0};
};
